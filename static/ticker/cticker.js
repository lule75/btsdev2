...
function update() {
	...
	r.onreadystatechange = function () {
		...
		if (r.status === 200) {
			...
			uiu_text(error_display, 'network error. Try again in' + (INTERVAL / 1000) + ' Sekunden ...');
		} else {
			cerror.silent('Ticker HTTP update failed with ' + r.status);
			reported = true;
			uiu_text(error_display, 'Fehler ' + r.status + ' This error is reported, when the values saved are compared with the active parameter set and a discrepancy is found. ');
		}
